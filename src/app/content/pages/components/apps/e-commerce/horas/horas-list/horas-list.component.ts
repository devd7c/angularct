...
import { HorasDataSource } from '../../_core/models/data-sources/horas.datasource';
...
@Component({
	selector: 'm-horas-list',
	templateUrl: './horas-list.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush
})
export class HorasListComponent implements OnInit {
	// Table fields
	dataSource: HorasDataSource;
	...
	// Selection
	selection = new SelectionModel<HoraModel>(true, []);
	horasResult: HoraModel[] = [];
	// Types
	...
	ngOnInit() {
		...
		**/
		...
		// Init DataSource
		this.dataSource = new HorasDataSource(this.horasService);
		let queryParams = new QueryParamsModel({});
		// Read from URL itemId, for restore previous state
		this.route.queryParams.subscribe(params => {
			if (params.id) {
				queryParams = this.horasService.lastFilter$.getValue();
				this.restoreState(queryParams, +params.id);
			}
			// First load
			queryParams.filter.empresa = this._idEmpresa;
			this.dataSource.loadHoras(queryParams);
		});
		this.dataSource.entitySubject.subscribe(res => this.horasResult = res);
	}
	...
	restoreState(queryParams: QueryParamsModel, id: number) {
		if (id > 0) {
			this.horasService.getHoraById(id).subscribe((res: HoraModel) => {
	...
	deleteHora(_item: HoraModel) {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.horasService.deleteHora(_item.id).subscribe(() => {
	...
	deleteHoras() {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.horasService.deleteHoras(idsForDeletion).subscribe(() => {
				this.layoutUtilsService.showActionNotification(_deleteMessage, MessageType.Delete);
				this.loadHorasList();
				this.selection.clear();
			});
	...
	updateSexoForHoras() {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.horasService.updateGeneroForHora(this.selection.selected, +res).subscribe(() => {
	...
	isAllSelected() {
		...
		const numRows = this.horasResult.length;
	...
	masterToggle() {
		if (this.isAllSelected()) {
			...
			this.horasResult.forEach(row => this.selection.select(row));
		}
	}
