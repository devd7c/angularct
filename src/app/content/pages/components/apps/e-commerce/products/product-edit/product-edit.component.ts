...
export class ProductEditComponent implements OnInit {
		...
		this.activatedRoute.queryParams.subscribe(params => {
			...
			if (id && id > 0) {
				this.productsService.getProductById(id).subscribe(res => {
					...
					this.oldProduct = Object.assign({}, res.data);
					this.initProduct();
				});
			} else {
	...
	loadLists() {
		...
		// console.log(this.remarksListState);
	...
	prepareProduct(): ProductModel {
		...
		_product.nombre = controls['nombre'].value;
	...
	getComponentTitle() {
		...
		result = `Edit product - ${this.product.manufacture} ${this.product.nombre}, ${this.product.modelYear}`;
		return result;
	}
