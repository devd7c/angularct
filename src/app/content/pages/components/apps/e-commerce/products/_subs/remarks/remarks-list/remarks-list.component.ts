...
export class RemarksListComponent implements OnInit {
	...
	displayedColumns = ['select', 'id', 'nombre', 'actions'];
	...
	clearAddForm() {
		...
		controls['newNombre'].setValue('');
		controls['newNombre'].markAsPristine();
		controls['newNombre'].markAsUntouched();
	...
	checkAddForm() {
		const controls = this.formGroup.controls;
		if (controls['newNombre'].invalid /*|| controls['newType'].invalid || controls['newDueDate'].invalid*/) {
			controls['newNombre'].markAsTouched();
	...
	saveNewRemark() {
		...
		this.remarkForAdd._isEditMode = false;
		this.remarkForAdd.nombre = controls['newNombre'].value;
	...
	clearEditForm() {
		const controls = this.formGroup.controls;
		controls['editNombre'].setValue('');
		// controls['editNombre'].markAsPristine();
		// controls['editNombre'].markAsUntouched();
	...
	checkEditForm() {
		const controls = this.formGroup.controls;
		if (controls['editNombre'].invalid /*|| controls['editType'].invalid || controls['editDueDate'].invalid*/) {
			// controls['editNombre'].markAsTouched();
	...
	editRemarkButtonOnClick(_item: ProductRemarkModel) {
		const controls = this.formGroup.controls;
		controls['editNombre'].setValue(_item.nombre);
	...
	saveUpdatedRemark(_item: ProductRemarkModel) {
		...
		const controls = this.formGroup.controls;
		this.loadingAfterSubmit = false;
		_item.nombre = controls['editNombre'].value;
	...
	filterConfiguration(): any {
		const filter: any = {};
		const searchNombre: string = this.searchInput.nativeElement.value;

		filter.nombre = searchNombre;
		return filter;
	}
	...
	fetchRemarks() {
		const messages = [];
		this.selection.selected.forEach(elem => { messages.push({ nombre: `${elem.nombre}`, id: elem.id }); });
