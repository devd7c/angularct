...
import { DomingosDataSource } from '../../_core/models/data-sources/domingos.datasource';
...
@Component({
	selector: 'm-domingos-list',
	templateUrl: './domingos-list.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush
})
export class DomingosListComponent implements OnInit {
	// Table fields
	dataSource: DomingosDataSource;
	...
	// Selection
	selection = new SelectionModel<DomingoModel>(true, []);
	domingosResult: DomingoModel[] = [];
	// Types
	...
	ngOnInit() {
		...
		**/
		...
		// Init DataSource
		this.dataSource = new DomingosDataSource(this.domingosService);
		let queryParams = new QueryParamsModel({});
		// Read from URL itemId, for restore previous state
		this.route.queryParams.subscribe(params => {
			if (params.id) {
				queryParams = this.domingosService.lastFilter$.getValue();
				this.restoreState(queryParams, +params.id);
			}
			// First load
			queryParams.filter.empresa = this._idEmpresa;
			this.dataSource.loadDomingos(queryParams);
		});
		this.dataSource.entitySubject.subscribe(res => this.domingosResult = res);
	}
	...
	restoreState(queryParams: QueryParamsModel, id: number) {
		if (id > 0) {
			this.domingosService.getDomingoById(id).subscribe((res: DomingoModel) => {
	...
	/** ACTIONS */
	/** Delete */
	deleteDomingo(_item: DomingoModel) {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.domingosService.deleteDomingo(_item.id).subscribe(() => {
	...
	deleteDomingos() {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.domingosService.deleteDomingos(idsForDeletion).subscribe(() => {
				this.layoutUtilsService.showActionNotification(_deleteMessage, MessageType.Delete);
				this.loadDomingosList();
				this.selection.clear();
			});
	...
	updateSexoForDomingos() {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.domingosService.updateGeneroForDomingo(this.selection.selected, +res).subscribe(() => {
	...
	isAllSelected() {
		...
		const numRows = this.domingosResult.length;
	...
	masterToggle() {
		if (this.isAllSelected()) {
			...
			this.domingosResult.forEach(row => this.selection.select(row));
		}
	}
