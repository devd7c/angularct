...
import { MultasDataSource } from '../../_core/models/data-sources/multas.datasource';
...
export class MultasListComponent implements OnInit {
	...
	// Selection
	selection = new SelectionModel<MultaModel>(true, []);
	multasResult: MultaModel[] = [];
	// Types
	...
	ngOnInit() {
		...
		**/
		...
		// Init DataSource
		this.dataSource = new MultasDataSource(this.multasService);
		let queryParams = new QueryParamsModel({});
		// Read from URL itemId, for restore previous state
		this.route.queryParams.subscribe(params => {
			if (params.id) {
				queryParams = this.multasService.lastFilter$.getValue();
				this.restoreState(queryParams, +params.id);
			}
			// First load
			queryParams.filter.empresa = this._idEmpresa;
			this.dataSource.loadMultas(queryParams);
		});
		this.dataSource.entitySubject.subscribe(res => this.multasResult = res);
	}
	...
	restoreState(queryParams: QueryParamsModel, id: number) {
		if (id > 0) {
			this.multasService.getMultaById(id).subscribe((res: MultaModel) => {
		...
		/*if ('empresa' in queryParams.filter) {
			this.filterEmpresa = queryParams.filter.empresa.toString();
		}*/
	...
	deleteMulta(_item: MultaModel) {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.multasService.deleteMulta(_item.id).subscribe(() => {
	...
	deleteMultas() {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.multasService.deleteMultas(idsForDeletion).subscribe(() => {
				this.layoutUtilsService.showActionNotification(_deleteMessage, MessageType.Delete);
				this.loadMultasList();
				this.selection.clear();
			});
	...
	updateSexoForMultas() {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.multasService.updateGeneroForMulta(this.selection.selected, +res).subscribe(() => {
	...
	isAllSelected() {
		...
		const numRows = this.multasResult.length;
	...
	masterToggle() {
		if (this.isAllSelected()) {
			...
			this.multasResult.forEach(row => this.selection.select(row));
		}
	}
