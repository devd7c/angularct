...
const API_PRODUCTS_URL = 'http://laravel.local/api/empresas';
// Real REST API
@Injectable()
export class ProductsService {
	...
	findProducts(queryParams: QueryParamsModel): Observable<QueryResultsModel> {
			...
			const url = API_PRODUCTS_URL + '/find/filter';
	...
	updateProduct(product: ProductModel): Observable<any> {
		// Note: Add headers if needed (tokens/bearer)
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.put(API_PRODUCTS_URL + `/${product.id}`, product, { headers: httpHeaders });
	}
