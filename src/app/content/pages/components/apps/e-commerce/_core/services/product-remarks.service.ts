...
const API_PRODUCTREMARKS_URL = 'http://laravel.local/api/sucursals';
// Real REST API
@Injectable()
export class ProductRemarksService {
	...
	getAllRemarksByProductId(productId: number): Observable<ProductRemarkModel[]> {
		const url = API_PRODUCTREMARKS_URL + '?idEmpresa=' + productId;
	...
	findRemarks(queryParams: QueryParamsModel, lastState: ListStateModel): Observable<ProductRemarkModel[]> {
		...
		const url = API_PRODUCTREMARKS_URL + '/find/filter';
		// Note: Add headers if needed (tokens/bearer)
		const httpHeaders = this.httpUtils.getHTTPHeaders();
	...
	updateRemark(remark: ProductRemarkModel): Observable<any> {
		// Note: Add headers if needed (tokens/bearer)
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.put(API_PRODUCTREMARKS_URL + `/${remark.id}`, remark, { headers: httpHeaders });
	}
